...
- [ ] **session-end-reminder `systemMessage` rendering** — spot-check on a live turn that the reminder surfaces to the model/operator as intended (docs consulted were a local copy; channel is correct-per-docs but the model-vs-user surface wasn't pinned).
