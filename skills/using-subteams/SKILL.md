...
**Model note:** All agents default to opus except doc-agent, developer, and ui-tester (sonnet). The two GPT critics (#13-14) run as sonnet harnesses that shell out to the Codex CLI (GPT) at high reasoning effort — see the `cross-review` skill for model/effort policy. The Gemini agents (#17-18) are likewise sonnet harnesses shelling out to the Antigravity CLI (`agy`) — the analyst at agy's fast default, the reviewer pinned to "Gemini 3.1 Pro (High)"; like the GPT critics they degrade gracefully (`agy` missing/unauthenticated → "gemini-unavailable", never a blocked pipeline), and a degraded lane is ALWAYS surfaced to the user, never silently absorbed. See model-selection skill for override guidance. When uncertain, ALWAYS choose opus — the cost difference is trivial compared to the cost of a wrong result.
