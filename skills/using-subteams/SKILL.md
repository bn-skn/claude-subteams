...
| 17 | gemini-analyst | sonnet (+Antigravity/Gemini) | Read, Grep, Glob, Bash | Multimodal media analysis — video frames with temporal precision, images (media Claude agents cannot open) — plus Gemini cross-model second opinion; shells out to the `agy` CLI headlessly | Gemini analysis report + caveats |
| 18 | gemini-code-reviewer | sonnet (+Antigravity/Gemini) | Read, Grep, Glob, Bash | OPTIONAL third-model review lane — dispatched only on explicit "triple review" request or top-tier (public/irreversible/security) changes at orchestrator discretion; never a silent default (see `cross-review` §1.8) | Structured Gemini findings + log-verified model |
| 19 | gemini-design-critic | sonnet (+Antigravity/Gemini) | Read, Grep, Glob, Bash | OPTIONAL cross-model design lane — structured Gemini critique of RENDERED visuals (screenshots, renders, PDF previews) alongside design-critic's code/spec review; opt-in per `design-qa` §6.5 | Severity-graded visual findings + log-verified model |

**Model note:** All agents default to opus except doc-agent, developer, and ui-tester (sonnet). The two GPT critics (#13-14) run as sonnet harnesses that shell out to the Codex CLI (GPT) at high reasoning effort — see the `cross-review` skill for model/effort policy. The Gemini agents (#17-19) are likewise sonnet harnesses shelling out to the Antigravity CLI (`agy`) — the analyst at agy's fast default, the reviewer and design-critic pinned to "Gemini 3.1 Pro (High)"; like the GPT critics they degrade gracefully (`agy` missing/unauthenticated → "gemini-unavailable", never a blocked pipeline), and a degraded lane is ALWAYS surfaced to the user, never silently absorbed. See model-selection skill for override guidance. When uncertain, ALWAYS choose opus — the cost difference is trivial compared to the cost of a wrong result.
...
- **Cross-model layer (when Codex is available):** Also dispatch `gpt-code-reviewer` + `gpt-devils-advocate` in parallel with the three Claude reviewers — full cross-model coverage with 4 critics. If Codex is unavailable (not on PATH, not authenticated, non-zero exit), proceed with the Claude critics only — never block the pipeline. See `claude-subteams:cross-review` for merge rules. A Gemini third lane (`gemini-code-reviewer`) exists but is opt-in only — explicit "triple review" request or top-tier public/irreversible change (cross-review §1.8); any requested-but-degraded lane is stated in the final summary.
